...
def main(seed):
    ...
    parser.add_argument('--teacher_model', type=str, default='/CIFAR100_7501.pt')
    ...
    data_test = torchvision.datasets.CIFAR10(args.data + args.dataset + '/', train=False, transform=transform_test,download=True)
    data_test_loader = torch.utils.data.DataLoader(data_test, batch_size=256, num_workers=0)
    ...
    if args.dataset == 'CIFAR10':
        ...
        student = resnet.ResNet18(num_classes=100).cuda()

    teacher.load_state_dict(torch.load(teacher_path))
    generator = gan.Generator(nz=args.latent_dim).cuda()
